...
var database, contacts_collection, statuses_collection, users_collection, files_collection,
  roles_with_statuses_collection, statuses_with_roles_collection, colors_collection;
var app = Express();
...
app.post("/verificationCurrentPassword", (request, response) => {
  ...
  users_collection.findOne({
    "UserName": logged_in_current_password.username,
    "Password": logged_in_current_password.current_password
  ...
  users_collection.findOne({"UserName": user.UserName}).then(function (mongo_user) {
    if (!mongo_user) {
      ...
      if (mongo_user.UserName == "Admin" && mongo_user.Name == "" && mongo_user.eMail == "" && mongo_user.Password == "") {
        ...
        users_collection.update({"UserName": "Admin"}, {
          $set: {
...
function check_exsisting_statuses_and_roles_and_files() {
  ...
  roles_with_statuses_collection.countDocuments(function (err, count) {
    if (!err && count === 0) {
      ...
      roles_with_statuses_collection.insertOne({
        Role: "תמיכה טכנית",
        Color: "#66ffff",
